        ...
        stage('Scan with Clair') {
            steps {
                script {
                    ...
                    sh 'docker run --rm --net=host -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/jenkins-workspace:/tmp objectiflibre/clair-scanner:latest --clair=$http://localhost:6060 --ip=localhost --log=debug rpdharanidhar/testing-nodeapp-jest:latest'
                }
            }
        }
