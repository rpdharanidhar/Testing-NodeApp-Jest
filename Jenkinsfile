        ...
        stage('Scan with Clair') {
            steps {
                script {
                    ...
                    sh 'docker run --rm --net=host -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/jenkins-workspace:/tmp objectiflibre/clair-scanner:latest --clair=$CLAIR_URL --ip=localhost --log=debug $IMAGE'
                }
            }
        }
